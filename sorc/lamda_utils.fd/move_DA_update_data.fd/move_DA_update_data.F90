!> @file
!! @brief Updates original-sized fv_core, fv_tracer first guess files
!! with analysis values from the larger files with the extra boundary rows.
!! Boundary row values with the GSI analysis are extracted from the larger
!! restart files and are put into a bndy file that is read by the LAM model.
