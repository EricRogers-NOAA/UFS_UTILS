...
!! It gets LAM grid attributes from the grid.tile7.halo3.nc
!! generated by the make_orog grid pre-preprocessing. This code 
!! is run after the LAM model execution in the DA cycle, and is part 
!! of the procedure to put the GSI analysis into the 00-h LAM bndy file.
...
!!
!! Input:  1) grid_specs.nc : Grid specs file output at the end of
!!            the model forecast (not the grid_specs file output from
!!            chgres_cube!)
!!         2) sfc_data.nc : Surface data netcdf file output in the 
!!            model RESTART directory at the end of the LAM DA 1-h
!!            forecast
!!         3) grid.tile7.halo3.nc: Fixed file with LAM grid attributes
!!
!! Output: 1) grid_spec_new.nc : Grid specs file with extra boundary
...
!
      integer,parameter :: num_dims_sfc=5                                  !< Number of dimensions in sfc_data.nc file (x,y,z1,z2,Time)                 
      integer,parameter :: num_dims_grid=5                                 !< Number of dimensions in grid_specs.nc file                                     
      integer,parameter :: halo=3                                          !< Halo depth                             
!
      integer :: i,iend_new,istart_new,j,jend_new,jstart_new,k,kend
!
!< i,j,k: Do loop indices
!! iend_new,istart_new,jend_new,jstart_new: The index limits of the
!!    original data in the enlarged domain 
!! kend: Vertical dimension position (3 in this code)
!
      integer :: n,na,natts,nd,ndims,nctype,ngatts,nvars,var_id
!
!< n,na: do loop counters
!! natts: Attributes for sfc_data variables
!! ndims: Number of dimensions in sfc_data or grid_spec files
!! nctype: netcdf file type
!! ngatts: # global attributes in original sfc_data.nc file
!! nvars: The total number of sfc_data file variables
!! var_id: Variable ID counter
!
      integer :: ncid_grid_spec_new,ncid_grid_spec_orig,ncid_grid_tile  &
                ,ncid_sfc_new,ncid_sfc_orig,nf_64bit_offset
!
!< ncid_grid_spec_new: Netcdf ID for new grid_spec file
!! ncid_grid_spec_orig : Netcdf ID for original grid_spec file
!! ncid_grid_tile : Netcdf ID for grid_tile7 grid attributes file
!! ncid_sfc_new : Netcdf ID for new sfc_data file
!! ncid_sfc_orig : Netcdf ID for original sfc_data file
!! nf_64bit_offset : Create a 64-bit offset format file (for large
!!                   Netcdf files
!
...
!
!< dimids: Number of dimensions in sfc_data or grid_specs files                       
!! dimid_sfc, dimid_grid: Dimensions in sfc_data or grid_specs files                       

      integer,dimension(:),allocatable :: dimsize_new                   &
                                         ,dimsize_orig
!
!< dimsize_new, dimsize_orig: Dimensions sizes of original and new                       
!! sfc_data.nc files
!
      real :: zero=0.
!
...
!
!< field, field_dbl: Placeholder arrays for reading fields 
!
      real,dimension(:),allocatable :: grid_point_count                    !< Counter for number of grid points in the new sfc_data, grid_spec files   
!
      character(len=50) :: name_att,name_var
!
!< name_att: Attribute name
...
!
      character(len=20),dimension(num_dims_sfc) :: dimname_sfc=(/          &  !< Dimension names in sfc_data.nc file
                                                                 'xaxis_1' &
                                                                ,'yaxis_1' &
                                                                ,'zaxis_1' &
                                                                ,'zaxis_2' &
                                                                ...
                                                                /)
!
      character(len=20),dimension(num_dims_grid) :: dimname_grid=(/        &  !< Dimension names in grid_specs.nc file                                  
                                                                 'grid_x'  &
                                                                ,'grid_y'  &
                                                                ,'time'    &
                                                                ,'grid_xt' &
