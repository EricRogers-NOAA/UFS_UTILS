!> @file
!! @brief Creates new sfc_data.nc and grid_specs.nc files with 
!! larger dimensions (extra boundary rows).
!! @authors Tom Black, Eric Rogers NCEP/EMC

!> This code reads the grid_spec.nc and sfc_data.nc files
!! generated at the end of a LAM forecast and creates larger
!! versions of these files with the extra boundary rows. 
!! This is run after the LAM model execution, and is part of the
!! procedure to put the GSI analysis into the 00-h LAM bndy file.
!! It learns the size of the enlarged grid and the lats/lons of the 
!! grid box centers and corners from the grid_spec.nc file
!! so that file also must be modified.  
!!
!> These larger files (fv_core, fv_tracer, sfc_data, grid_spec) are 
!! all input to the LAM GSI analysis so that the analysis is done
!! on the extra boundary row points.
...
!
      integer,parameter :: num_dims_sfc=5
...
!
      integer :: ncid_grid_spec_new,ncid_grid_spec_orig,ncid_grid_tile  &
                ,ncid_sfc_new,ncid_sfc_orig,nf_64bit_offset
!
...
!-----------------------------------------------------------------------
!
      call check(nf90_create(filename_sfc_data_new,                     &
                  or(nf90_clobber,nf90_64bit_offset),ncid_sfc_new))
!
!---------------
!*** Dimensions
!---------------
